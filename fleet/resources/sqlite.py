...
class SyncSnapshotDiff:
    ...
    def expect_only_v2(
        ...
        if self.use_structured_diff and resource.client is not None and resource._mode == "http":
            ...
            if api_diff is not None:
                self._validate_diff_against_allowed_changes_v2(api_diff, allowed_changes)
        ...
        # Fall back to full diff for complex cases
        diff = self._collect()
        self._validate_diff_against_allowed_changes_v2(diff, allowed_changes)
        if require_completeness:
            self._verify_completeness_v2(allowed_changes)
        return self
    ...
    def _verify_completeness_v2(self, allowed_changes: List[Dict[str, Any]]):
        ...
        def check_spec(table: str, spec: Dict[str, Any], pk_columns: List[str]):
            try:
                pk = spec.get("pk")
                where_sql = self._build_pk_where_clause(pk_columns, pk)
                select_sql = f"SELECT * FROM {_quote_identifier(table)} WHERE {where_sql}"
                after_response = self.after.resource.query(select_sql)
                after_row = (
                    dict(zip(after_response.columns, after_response.rows[0]))
                    if after_response.rows
                    else None
        ...
        checks: List[Tuple[str, Dict[str, Any], List[str]]] = []
        for table, table_specs in specs_by_table.items():
            pk_columns = self._get_primary_key_columns(table)
            for spec in table_specs:
                checks.append((table, spec, pk_columns))

        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(check_spec, table, spec, pk_columns)
                for table, spec, pk_columns in checks
            ]
            concurrent.futures.wait(futures)
