...
class SyncFilesystemDiff:
    """Wraps a filesystem diff response with assertion helpers."""

    def __init__(self, response: FsDiffResponse, resource: "FilesystemResource"):
    ...
    def expect_no_changes(self) -> "SyncFilesystemDiff":
    ...
    def _validate_entry(
        ...
        if "doc_text" in spec or "doc_text_contains" in spec:
            try:
                doc_text = self._resource.doc_text(path)
...
class FilesystemResource(Resource):
    ...
    def doc_text(self, path: str, max_size: int = 10485760) -> str:
    ...
    def doc_metadata(self, path: str) -> DocMetadataResponse:
    ...
    def doc_structured(self, path: str) -> DocStructuredResponse:
