...
"""LLM-as-a-judge grading module for Fleet SDK.

Supports customer-provided LLM endpoints (Anthropic or OpenAI format)
for evaluation grading in airgapped environments.
"""
...
class Criterion(BaseModel):
    """A single evaluation criterion with scoring levels."""

    name: str
    max_score: int
    levels: Dict[int, str] = Field(
        default_factory=dict,
        description="Mapping of score -> description for each level",
    )


class Rubric(BaseModel):
    """Evaluation rubric containing criteria and optional task context."""
...
class JudgeResult(BaseModel):
    ...
    scores: Dict[str, float] = Field(
        description="Criterion name -> score",
    )
    total: float
    max_total: float
    ...
    reasoning: Dict[str, str] = Field(
        default_factory=dict,
        description="Criterion name -> reasoning text",
    )


_SYSTEM_PROMPT = (
    "You are an expert evaluator. You will be given a submission to evaluate "
    "against a rubric with specific criteria. For each criterion, assign a score "
    "and provide reasoning.\n\n"
    "You MUST respond with valid JSON in exactly this format:\n"
    '{"scores": {"criterion_name": {"score": <integer>, "reasoning": "..."}, ...}}\n\n'
    "Only output the JSON object, nothing else."
)


class JudgeService:
    """Synchronous LLM-as-a-judge grading service."""
    ...
    def grade(
        ...
        if self.config.api_format == "anthropic":
            raw = self._call_anthropic(messages)
        else:
...
def get_judge_config() -> Optional[JudgeEndpointConfig]:
    """Read judge configuration from environment variables.

    Returns None if FLEET_JUDGE_ENDPOINT is not set.
    """
    url = os.environ.get("FLEET_JUDGE_ENDPOINT")
    if not url:
        return None

    return JudgeEndpointConfig(
        url=url,
        api_key=os.environ.get("FLEET_JUDGE_API_KEY", ""),
        model=os.environ.get("FLEET_JUDGE_MODEL", "claude-sonnet-4-20250514"),
        api_format=os.environ.get("FLEET_JUDGE_API_FORMAT", "anthropic"),  # type: ignore[arg-type]
    )
