...
async def test_new_format():
    ...
    # List categories
    categories = flt.instance.list_categories()
    print(f"Available categories: {categories}")
    
    # List environments in browser category
    browser_envs = flt.instance.list_names("browser")
    print(f"Browser environments: {browser_envs}")
    
    # List versions for chrome-desktop
    versions = flt.instance.list_versions("browser", "chrome-desktop")
    print(f"Chrome desktop versions: {versions}")
    
    # List all available environments
    all_envs = flt.instance.list_environments()
    ...
    for spec in test_specs:
        supported = flt.instance.is_environment_supported(spec)
