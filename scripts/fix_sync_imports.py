...
def fix_file(filepath: Path) -> bool:
    ...
    if depth == 0:
        # Files in fleet/ root (like client.py), use single dot
        content = content.replace('from fleet.verifiers', 'from .verifiers')
    elif depth == 1:
        # Files in direct subdirectories of fleet/ (like fleet/instance/), use double dot
        content = content.replace('from fleet.verifiers', 'from ..verifiers')
    elif depth == 2:
        # Files in sub-subdirectories (like fleet/instance/subdir/), use triple dot
        content = content.replace('from fleet.verifiers', 'from ...verifiers')
