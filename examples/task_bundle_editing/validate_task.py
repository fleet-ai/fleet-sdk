...
def validate(bundle_dir: Path, new_key: str | None = None) -> list[str]:
    ...
    if verifier:
        ...
        if not code:
            ...
            if tree and code:
                # Detect whether the verifier fetches agent outputs at all.
                # For external (remote-env) verifiers the working pattern is
                # env.fs().client.request("GET", "/fs/file/get", ...) plus
                # File.from_base64 / Image.from_base64. File.from_env is also
                # acceptable for in-container verifier runtimes.
                ...
                # For remote-env verifiers, File.from_env / Image.from_env don't
                # reliably fetch /workdir/output/* across containers. The working
                # pattern uses env.fs().client.request("GET", "/fs/file/get") to
                # pull raw bytes and wrap them with File.from_base64 / Image.from_base64.
                reads_outputs = (
                    "fs/file/get" in code
                    or "File.from_base64" in code
                    or "Image.from_base64" in code
                    or "File.from_env" in code
                    or "Image.from_env" in code
                )
                if likely_outputs and not reads_outputs:
                    warnings.append(
                        f"Prompt asks agent to produce files "
                        f"({', '.join(sorted(likely_outputs)[:5])}) but verifier "
                        f"does not appear to fetch them. Use "
                        f"env.fs().client.request('GET', '/fs/file/get', "
                        f"params={{'path': '/workdir/output/<file>'}}) and wrap "
                        f"with File.from_base64 / Image.from_base64."
                    )
