...
from fleet import Fleet, verifier_sync, TASK_SUCCESSFUL_SCORE, Task

# Constants for task failure
TASK_FAILED_SCORE = 0.0


@verifier_sync(key="create_bug_issue_v1")
def create_bug_issue(env, project_key: str = "SCRUM", issue_title: str = "Sample Bug") -> float:
    ...
    try:
        ...
        if result.rows and len(result.rows) > 0:
            ...
            print(f"✗ No bug issue found with title '{issue_title}' in project {project_key}")
...
def main():
    ...
    try:
        ...
        env = fleet_client.make("fira:v1.3.1")
        ...
        result = task.verifier(env, project_key="SCRUM", issue_title="Login button not working")
        ...
        try:
            remote_result = task.verifier.remote(env, project_key="SCRUM", issue_title="Login button not working")
            print(f"  Remote check result: {remote_result}")
            print(f"  Remote execution {'✓ matches local' if remote_result == result else '✗ differs from local'}")
        except Exception as e:
        ...
        try:
            ...
            print(f"  Remote check result: {remote_result}")
            print(f"  Remote execution {'✓ matches local' if remote_result == result else '✗ differs from local'}")
            print(f"  Task {'completed successfully' if remote_result == TASK_SUCCESSFUL_SCORE else 'failed'} (remote check)!")
        except Exception as e:
            print(f"  ✗ Remote execution failed: {e}")
        print()
